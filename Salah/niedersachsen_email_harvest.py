...
def load_excluded_emails() -> Set[str]:
    """Load emails from main app folder that should be excluded from harvesting."""
    excluded: Set[str] = set()
    parent_dir = Path(__file__).parent.parent  # Go up to main auto folder
    
    # Check applications_sent.csv in parent directory
    sent_file = parent_dir / "applications_sent.csv"
    if sent_file.exists():
        try:
            ...
            if 'Email' in df.columns:
                for email in df['Email'].dropna().astype(str):
                    ...
                    if email_clean and '@' in email_clean:
                        excluded.add(email_clean)
            print(f"📋 Excluded {len(excluded)} emails from applications_sent.csv")
        except Exception as e:
            print(f"⚠️ Could not load excluded emails from parent: {e}")
    
    # Check any existing harvested CSV files in parent
    for pattern in ["niedersachsen_*.csv", "*_2000_emails.csv", "*emails.csv"]:
        for csv_file in parent_dir.glob(pattern):
            if csv_file.name == "applications_sent.csv":
                continue
            try:
                ...
                if 'Email' in df.columns:
                    for email in df['Email'].dropna().astype(str):
                        ...
                        if email_clean and '@' in email_clean:
                            excluded.add(email_clean)
                print(f"📋 Excluded {len(excluded)} emails from {csv_file.name}")
            except Exception:
                continue
...
def main() -> None:
    ...
    # Load emails to exclude from main app
    ...
    rows: List[dict] = []
    seen_emails: Set[str] = set(excluded_emails)  # Start with excluded emails from main app
    if args.output.exists():
        try:
            ...
            print(f"Loaded {len(seen_emails)} existing emails (including {len(excluded_emails)} from main app)")
        except Exception as exc:
